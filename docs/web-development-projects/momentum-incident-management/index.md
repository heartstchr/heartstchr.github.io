...
project:
  ...
  perspective: {"executive":"An incident management product that gives triage teams a polished, role-based portal over a clean REST API — with search, sorting, and export where operations teams actually live.","technical":"Vue 3 + PrimeVue front end against an expressive REST API; role-based access, incident lifecycle states, notes and task tracking, and exportable sorted/searchable views."}
